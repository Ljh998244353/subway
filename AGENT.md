...
- 当前阶段：开发前准备已完成，P0 项目基线尚未开始。
- 当前代码状态：尚无 `frontend/`、`backend/`、`ai-services/`、`infra/` 或自动化测试工程。
- 当前已有交付：项目规划文档、Codex 项目 skill、第三方声明、Typst 汇报稿和已生成的 `slides/slide.pdf`。
- 开发模型：单 AI 增量迭代开发。`P0` 到 `P12` 是路线图，实际工作按 `P0-I1`、`P0-I2` 这类小增量逐步交付。
- 下一步：执行 `P0-I1`，只创建项目边界、目标用户、非目标范围和合规红线相关文档，不直接开始写前端或后端。
...
3. 不跳阶段、不做大爆炸开发。当前 `P0-I1` 未完成前，不创建前端、后端、AI 视频服务或部署工程。
...
10. 每个增量结束必须更新受影响的 `context/*.md` 和 `PROGRESS.md`，并在 `PROGRESS.md` 提供给人类复制使用的下一步 AI prompt。
...
1. 读取必读文件
2. 总结当前状态
3. 识别当前增量：优先 context/TODO_NEXT.md，其次 PROGRESS.md 的推荐 prompt，其次最早未完成增量
4. 自动选择一个主角色模式
5. 写明本次小交付和明确不做的范围
...
11. 留出下一次给人类使用的 AI prompt
```

## 自动角色选择规则

```text
需求、范围、用户故事、指标、验收、进度 prompt -> Product Mode
...
本次自动识别当前增量；如果没有 context，则进入 Product Mode，执行 P0-I1 项目边界与合规基线。

只创建或更新 P0-I1 需要的 docs/ 与 context/ 文件。
不要写前端、后端、AI 视频服务或部署工程。
检查新增内容是否涉及非工程问题、付费工具、经济成本、第三方素材、许可证、隐私或潜在侵权风险。
完成后更新 PROGRESS.md；如果发现高风险事项，同步更新 IMPORTANT.md。
```
...
相关测试或检查已运行
PROGRESS.md 已更新
PROGRESS.md 已写入给人类复制使用的下一步 AI prompt
context/ 已按阶段要求更新或明确尚未进入 P0
```
