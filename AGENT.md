...
- 当前阶段：P0 项目基线与上下文恢复、P1 设计规范阶段、P2-I1 前端工程初始化、P2-I2 Mock 数据和共享类型已完成；下一步进入 P2-I3 运营总览页面。
- 当前代码状态：已有 `frontend/` React + TypeScript + Vite 工程骨架、5 个核心路由占位、共享 TypeScript 类型、虚构 Mock 数据和数据边界测试；尚无 `backend/`、`ai-services/`、`infra/` 或 CI。
- 当前已有交付：项目规划文档、P0 需求与系统设计文档、P1 设计规范文档、P2-I1 前端工程骨架、P2-I2 Mock 数据与共享类型、`context/` 恢复包、Codex 项目 skill、第三方声明、Typst 汇报稿和已生成的 `slides/slide.pdf`。
...
- 下一步：执行 `P2-I3`，基于 `frontend/src/mock` 和 `frontend/src/types` 实现 `/dashboard` 运营总览页面。不要创建后端、AI 视频服务或部署工程，不接真实 API、真实视频或真实商场素材。
...
3. 不跳阶段、不做大爆炸开发。当前下一步是 `P2-I3`，只实现运营总览页面及其必要组件/测试，不要提前实现其他业务页面，也不要创建后端、AI 视频服务或部署工程。
...
AI 必须据此读取 `PROGRESS.md` 和 `context/TODO_NEXT.md`，自动识别当前应执行 `P2-I3`，并完成运营总览页面、必要测试、风险记录和上下文更新。
