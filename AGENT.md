...
- 当前阶段：P0 项目基线与上下文恢复、P1 设计规范阶段、P2-I1 前端工程初始化、P2-I2 Mock 数据和共享类型、P2-I3 运营总览页面、P2-I4 店铺分析页面、P2-I5 低效预警页面、P2-I6 数字孪生 Demo 页面、P2-I7 E2E/响应式/演示打磨、P2-I8 客群画像页面、P2-I9 CP2 前端 Demo 收口和交接已完成；下一步进入 P3-I1 工程化骨架规划与质量门禁对齐。
- 当前代码状态：已有 `frontend/` React + TypeScript + Vite 工程骨架、5 个核心路由、共享 TypeScript 类型、虚构 Mock 数据、数据边界测试、核心演示流转测试、CP2 演示就绪测试、响应式 CSS 检查和 `/dashboard`、`/store-analysis`、`/store-alerts`、`/digital-twin`、`/customer-profile` 五个业务页面；尚无 `backend/`、`ai-services/`、`infra/` 或 CI。
- 当前已有交付：项目规划文档、P0 需求与系统设计文档、P1 设计规范文档、P2-I1 前端工程骨架、P2-I2 Mock 数据与共享类型、P2-I3 运营总览页面、P2-I4 店铺分析页面、P2-I5 低效预警页面、P2-I6 数字孪生 Demo 页面、P2-I7 QA 打磨与核心路径检查、P2-I8 客群画像页面、P2-I9 前端 Demo 交接文档和演示就绪测试、`context/` 恢复包、Codex 项目 skill、第三方声明、Typst 汇报稿和已生成的 `slides/slide.pdf`。
...
- 下一步：执行 `P3-I1`，做工程化骨架规划与质量门禁对齐。优先定义根级质量门禁、CI 检查计划和 P3 拆分；不要直接创建后端或 AI 视频服务，不接真实 API、真实视频、真实商场平面图、真实商场素材或个人轨迹。
...
3. 不跳阶段、不做大爆炸开发。当前下一步是 `P3-I1`，只做工程化骨架规划与质量门禁对齐；除非该增量明确拆出边界，不要提前创建后端、AI 视频服务或生产部署工程。
...
AI 必须据此读取 `PROGRESS.md` 和 `context/TODO_NEXT.md`，自动识别当前应执行 `P3-I1`，并完成工程化骨架规划、质量门禁对齐、风险记录和上下文更新。
