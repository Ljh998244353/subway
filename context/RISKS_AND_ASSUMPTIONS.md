...
| BlenderMCP can execute Blender Python and optionally use telemetry or external asset/generation APIs | High | Approved only for local synthetic modeling; telemetry must stay disabled, and Sketchfab/Poly Haven/Hyper3D/Hunyuan3D/Rodin/Fal/downloaded assets remain blocked until separate audit |
...
P8-I1 productized frontend lazy loading, manual vendor chunking, route loading/error fallback, and 3D render-loop optimization without adding dependencies, assets, real data, or deployment infrastructure. The next increment should start the store score MVP backend contract using synthetic/demo aggregates only, while still avoiding real MySQL, real mall material, external services, production data, and real video integration by default.
