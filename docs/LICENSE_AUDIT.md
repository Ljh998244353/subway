...
BlenderMCP / `ahujasid/blender-mcp` was originally recorded as a candidate local automation bridge. On 2026-05-26 the user approved controlled local use and provided a local copy at `L:\\Software\\blender-mcp` (README label 1.5.5, MIT in `pyproject.toml`). The README and source include arbitrary Blender Python execution, telemetry controls, and optional external asset/generation integrations. Therefore the project decision is: allowed only for controlled localhost synthetic modeling; telemetry must be disabled with `DISABLE_TELEMETRY=true` / `BLENDER_MCP_DISABLE_TELEMETRY=true` / `MCP_DISABLE_TELEMETRY=true`; Sketchfab, Poly Haven, Hyper3D, Hunyuan3D, Rodin, Fal, downloaded models, external textures, real mall material, and real brand assets remain blocked until separate audit.
...
## P7-R2 GLB recovery and self-authored asset audit

P7-R2 uses already-adopted `three`, `@react-three/fiber`, and audited `@react-three/drei@10.7.7` for GLB loading, HTML scene labels, and OrbitControls. `@react-three/drei` is MIT-licensed in the pmndrs ecosystem and is allowed only for the self-authored synthetic mall model workflow.
