...
| BlenderMCP / blender-mcp | https://github.com/ahujasid/blender-mcp | README label 1.5.5; candidate only, not installed | MIT License per GitHub metadata | Candidate local MCP bridge for AI-assisted Blender automation of self-authored synthetic mall geometry and GLB/GLTF export helpers | MIT generally permits commercial use, but the tool can run Blender Python and must be treated as a local code-execution bridge | Preserve MIT license notice if adopted; disable telemetry; do not use external asset APIs or downloaded models without separate audit | Candidate only; needs explicit user approval before install/use |
...
| zustand | https://github.com/pmndrs/zustand | 5.0.13 | MIT | Transitive state-store dependency of R3F | MIT generally permits commercial use with license notice | Preserve license notice | Allowed |
