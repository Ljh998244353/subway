...
test('digital twin navigation exposes breadcrumbs, return paths, and quick entries', () => {
  ...
  assert.match(commandBar, /模型源切换/);
  assert.match(commandBar, /复制场景链接/);
  assert.match(commandBar, /打开场景/);
  const gradingBoard = read('../src/components/dashboard/MerchantGradingBoard.tsx');
  assert.match(gradingBoard, /评分榜楼层筛选/);
  assert.match(gradingBoard, /评分榜业态筛选/);
  assert.match(gradingBoard, /评分榜评级筛选/);
  assert.match(gradingBoard, /评分榜最低分筛选/);
  assert.match(gradingBoard, /暂无符合筛选的合成店铺/);
...
test('digital twin viewport overlay is an actionable navigation surface', () => {
  ...
  assert.match(viewport, /3D 在线 ·/);
  assert.match(viewport, /GLB 原型预览/);
  assert.match(viewport, /PrototypeGlbViewport/);
  assert.match(viewport, /合成场景读数/);
  assert.match(viewport, /场景密度/);
  const prototypeViewport = read('../src/components/twin-engine/PrototypeGlbViewport.tsx');
  assert.match(prototypeViewport, /自建 GLB 原型审阅模式/);
  assert.match(prototypeViewport, /加载自建三层原型/);
  assert.match(prototypeViewport, /synthetic only/);
  assert.match(prototypeViewport, /SCENARIO_DENSITY_LABEL/);
  assert.match(fallback, /TwinLevelNavigation/);
  assert.match(fallback, /SVG 数字孪生 F1 平面图/);
  assert.match(fallback, /2D 合成场景/);
  assert.match(fallback, /Synthetic SVG incident emphasis/);
  assert.match(fallback, /DENSITY_MULTIPLIER/);
  assert.match(threeViewport, /const positions = new Float32Array\(count \* 3\)/);
  assert.match(threeViewport, /<shaderMaterial/);
  assert.match(threeViewport, /DENSITY_MULTIPLIER/);
  assert.match(threeViewport, /speedMultiplier=\{scenarioSpeed\}/);
  assert.match(threeViewport, /SyntheticScenarioIncidentEmphasis/);
...
test('digital twin shell separates overview cockpit from full-screen detail pages', () => {
  ...
  assert.match(shell, /AnalyticsSnapshotPanel/);
...
test('digital twin workspace keeps timeline and command bar out of large viewport overlays', () => {
  ...
  assert.match(timeline, /客流场景/);
  assert.match(timeline, /回放速度/);
  assert.match(timeline, /Synthetic incident intensity/);
  assert.match(timeline, /setState\(\{ scenarioDensity \}\)/);
  assert.match(timeline, /setState\(\{ scenarioSpeed \}\)/);
  assert.match(timeline, /setState\(\{ incidentLevel: level \}\)/);
...
test('inspector panels use compact sections and rows instead of repeated heavy cards', () => {
  ...
  const analytics = read('../src/components/dashboard/AnalyticsSnapshotPanel.tsx');
  ...
  assert.match(analytics, /P9 Synthetic Analytics/);
  assert.match(analytics, /综合体分析快照/);
  assert.match(analytics, /匿名聚合快照/);
  assert.match(analytics, /modeInsightLabel/);
  assert.match(analytics, /热区解释/);
  assert.match(analytics, /动线解释/);
  assert.match(analytics, /评分解释/);
  assert.match(analytics, /告警解释/);
  assert.match(analytics, /回放上下文/);
  assert.match(analytics, /scrubberMinute/);
  assert.match(analytics, /scenarioSpeed/);
  assert.match(analytics, /scenarioDensity/);
  assert.match(analytics, /mockCustomerProfile/);
  assert.match(analytics, /mockHeatmapPoints/);
  assert.match(analytics, /mockFlowEdges/);
